<script lang="ts">
  ...
  export let recipeId: string = null;

  const dispatch = createEventDispatcher();

  const mainPictureSchema = yup
    .mixed()
    .fileMax({
      maxBytes: 50000,
  ...
  if (type === 'ADD') {
    mainPictureSchema.required('Picture Required');
  }
  ...
  const { form, errors, handleChange, handleSubmit, updateValidateField } =
    createForm({
      ...
      onSubmit: async (values) => {
        try {
          recipeId = await saveRecipe(values, $authStore.user.uid, recipeId);
          await dispatch('saved-' + type, { id: recipeId });
          return;
        } catch (e) {
          alert('error saving');
          console.log(e);
...
<Row>
  <Col>
    <h1>{type === 'ADD' ? 'Add' : 'Edit'} Recipe</h1>
  </Col>
</Row>
